# fix-commit-msg.ps1
# Rewrites the commit message of e8ad3d4 (HEAD~1) using filter-branch.
# Safe: private repo, no collaborators.
...
$badHash = "e8ad3d4c356062ab62893249096f688da2602de1"

# Write the correct message to a temp file
$tmpFile = [System.IO.Path]::GetTempFileName()
[System.IO.File]::WriteAllText($tmpFile, $correctMessage, [System.Text.Encoding]::UTF8)

Write-Host "Rewriting commit $badHash ..."
$env:FILTER_BRANCH_SQUELCH_WARNING = "1"
git filter-branch -f --msg-filter "if (`$GIT_COMMIT -eq '$badHash') { cat '$($tmpFile -replace '\\','/')' } else { cat }" HEAD~1..HEAD
...
git log --oneline -3
Write-Host ""
git show HEAD~1 --format="HASH: %H%nSUBJECT: %s%nBODY:%n%b" --no-patch
